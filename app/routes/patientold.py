...
def baseline_checkin():
    ...
    patient: PatientProfile = PatientProfile.query.filter_by(
        user_sid=current_user.sid
    ).first()

    if not patient:
        current_app.logger.error(
            "[BaselineCheckin] Patient profile not found for user %s", current_user.sid
        )
    ...
    if patient.onboarding_complete:
        current_app.logger.info(
            "[BaselineCheckin] Patient %s already onboarded. Redirecting.", patient.sid
        )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            current_app.logger.debug(
                "[BaselineCheckin] WellnessCheck object created for %s", patient.sid
            )
            ...
            if conditions:
                current_app.logger.debug(
                    "[BaselineCheckin] Conditions received: %s", conditions
                )
            ...
            if preferences:
                current_app.logger.debug(
                    "[BaselineCheckin] Preferences received: %s", preferences
                )
            ...
            current_app.logger.info(
                "[BaselineCheckin] Marking patient %s onboarding_complete=True",
                patient.sid,
            )
            ...
            db.session.commit()
            current_app.logger.info(
                "[BaselineCheckin] Commit successful for patient %s", patient.sid
            )
            ...
            current_app.logger.exception(
                "[BaselineCheckin] SQLAlchemyError: %s", db_err
            )
    ...
    # ------------------------------------------------------------
    current_app.logger.info(
        "[BaselineCheckin] Rendering baseline form for patient %s", patient.sid
    )
...
def patient_dashboard():
    ...
    public_profile_data = {
        ...
        "can_view_afflictions": (
            user.can_be_seen_field("afflictions", viewer_is_friend=viewer_is_friend)
            if user
            else False
        ),
        "can_view_qol": (
            user.can_be_seen_field("qol_scores", viewer_is_friend=viewer_is_friend)
            if user
            else False
        ),
        "can_view_voting": (
            user.can_be_seen_field("voting_history", viewer_is_friend=viewer_is_friend)
            if user
            else False
        ),
...
def stop_using_product(usage_id):
    ...
    patient_usage = (
        PatientProductUsage.query.filter_by(
            sid=usage.sid, product_id=usage.product_id, still_using=True
        )
...
def checkins_hub():
            ...
            last_products.append(
                {
                    ...
                    "product_name": (
                        getattr(p.product, "name", "Unknown")
                        if p.product
                        else "Unknown"
                    ),
        ...
        usage_json = [
            {
                ...
                "count": getattr(u, "times_per_day", 1),
            }
    ...
    return render_template(
        ...
        wellness_history_json=[
            wc.to_dict() for wc in profile.wellness_checks
        ],  # Optional
...
def submit_wellness_check():
    ...
    if last_checkin:
        checkin.pct_change_qol = checkin.overall_qol - getattr(
            last_checkin, "overall_qol", 0.0
        )
    else:
    ...
    total_qol_delta = getattr(
        checkin, "overall_qol_delta", checkin.pct_change_qol
    )  # fallback
    cannabis_contribution = total_qol_delta * (cannabis_pct / 100)
...
def onboarding_submit():
    ...
    wellness_entry = (
        PatientWellness.query.filter_by(sid=sid)
        .order_by(PatientWellness.created_at.desc())
        .first()
    )

    if not wellness_entry:
        # First-time baseline
        new_entry = PatientWellness(sid=sid, **baseline_data, baseline=True)
...
def product_list():
    ...
    q = (request.args.get("q") or "").strip()
    classification = (
        request.args.get("classification") or request.args.get("strain") or ""
    ).strip()
    ...
    if affliction:
        if hasattr(Product, "affliction"):
            ...
            try:
                ...
                qry = qry.join(ProductAffliction).filter(
                    ProductAffliction.name.ilike(affliction)
                )
            except Exception:
                pass
        ...
        for fld in ("is_available", "available", "status"):
            if hasattr(Product, fld):
                if fld == "status" and available:
                    qry = qry.filter(
                        getattr(Product, fld).in_(["active", "approved", "published"])
                    )
    ...
    if sort == "thc_desc" and thc_col is not None:
        ...
        order_col = (
            ...
            or getattr(Product, "datetime", None)
            or Product.id
        )
...
@patient_bp.route(
    "/product/grassroots/new",
    methods=["GET", "POST"],
    endpoint="product_grassroots_new",
)
@login_required
@role_required(UserRoleEnum.PATIENT)
def product_grassroots_new():
    ...
    if not name:
        ...
        return redirect(
            url_for(
                "patient.product_grassroots_new",
                next=_nx(),
    ...
    checkin_ep = (
        "patient.checkin_entry"
        if "patient.checkin_entry" in current_app.view_functions
        else (
            "patient.checkins_hub"
            if "patient.checkins_hub" in current_app.view_functions
            else "patient."
        )
    )
    return redirect(
        _safe(
            checkin_ep,
            new_product_id=getattr(p, "id", None),
            new_product_name=getattr(p, "name", name),
        )
    )
...
def _compute_qol_improvement_and_upvotes(
    product_id: int,
    *,
    window_hours: int = 8,
...
def product_detail(product_id):
    ...
    preferred_dispensary, nearby_dispensaries = get_nearby_dispensaries(
    ...
    if "Upvote" in globals():
        try:
            vote_count = Upvote.query.filter_by(
...
def get_nearby_dispensaries(patient, product, radius_miles=25, max_results=5):
    ...
    """
    from app.models import Dispensary, InventoryReport
    ...
    for inv in inventory_rows:
        ...
        dist = (
            distance_miles(patient_coords, disp_coords)
            if (patient_coords and disp_coords)
            else None
        )
...
@patient_bp.route(
    "/products/<int:product_id>/edit", methods=["GET", "POST"], endpoint="product_edit"
)
@login_required
@role_required(UserRoleEnum.PATIENT)
def product_edit(product_id: int):
    ...
    if _is_locked(p):
        flash(
            "This product has been approved and can no longer be crowd-edited.", "info"
        )
    ...
    if img and getattr(img, "filename", None):
        ...
        if saved:
            ...
            flash(
                "Could not save image (allowed: png, jpg, jpeg, gif, webp).", "warning"
            )
...
@patient_bp.route(
    "/products/my_submissions", methods=["GET"], endpoint="product_mysubmissions"
)
@login_required
@role_required(UserRoleEnum.PATIENT)
    ...
    if Product and uid is not None:
        ...
        if conds:
            order_col = (
                getattr(Product, "created_at", None)
                or getattr(Product, "datetime", None)
                or getattr(Product, "created", None)
                or Product.id
            )
            rows = Product.query.filter(or_(*conds)).order_by(order_col.desc()).all()
    return render_template(
        "patient/product_mysubmissions.html", submitted_products=rows
    )
...
def _fetch_profile():
    try:
        ...
        return PatientProfile.query.filter_by(
...
@patient_bp.route(
    "/products/<int:usage_id>/update", methods=["POST"], endpoint="update_product_usage"
)
@login_required
@role_required(UserRoleEnum.PATIENT)
def update_product_usage(usage_id):
    ...
    if dosage:
        usage.dosage_mg = dosage
    if frequency:
        usage.times_per_day = frequency
...
@patient_bp.route(
    "/products/confirm", methods=["POST"], endpoint="confirm_product_list"
)
@login_required
@role_required(UserRoleEnum.PATIENT)
...
def record_save_mandatory():
    ...
    if not all([name, alias, email, zip_code, password]):
        return (
            jsonify({"ok": False, "error": "All mandatory fields must be filled"}),
            400,
        )
    ...
    try:
        ...
        current_user.alias_slug = (
            re.sub(r"[^a-z0-9\-]+", "-", alias.lower()).strip("-") or "user"
        )
        ...
        if password:
            current_user.set_password(
                password
            )  # assuming your User model has set_password()
        db.session.add(profile)
...
def record_save_demographics():
    ...
    try:
        ...
        profile.cannabis_use_characterization = data.get(
            "cannabis_use_characterization"
        )
        db.session.add(profile)
        db.session.commit()
        return jsonify({"ok": True})
...
def settings_dashboard():
    ...
    return render_template(
        "patient/settings_dashboard.html", profile=profile, prefs=prefs
    )
...
def get_support_models():
    ...
    """
    from app.models import GroupMember, SupportGroup
...
def groups(group_key):
    ...
    try:
        if L:
            ...
            links = q.order_by(
                getattr(L, "created_at", None)
                or getattr(L, "timestamp", None)
                or _dt.utcnow()
            ).all()
    except Exception:
        links = []
...
def group_join():
    raw = (
        request.form.get("group_key")
        or request.form.get("group_id")
        or (request.json.get("group_key") if request.is_json else None)
    )
    ...
    if not key or not sid:
        ...
        return redirect(
            _safe_next_url(explicit=next_url, default_endpoint="patient.groups")
        )
    ...
    try:
        ...
        if hasattr(m, "group_key"):
            m.group_key = key
        if hasattr(m, "group_name"):
            m.group_name = name
        # Fallback to numeric:
        ...
        db.session.rollback()
        try:
            current_app.logger.exception("group_join failed: %s", e)
        except Exception:
            pass
...
def group_leave():
    ...
    try:
        ...
        db.session.rollback()
        try:
            current_app.logger.exception("group_leave failed: %s", e)
        except Exception:
            pass
...
def group_post():
    ...
    try:
        ...
        for u in ("sid", "user_sid", "patient_sid"):
            if hasattr(row, u):
                setattr(row, u, _user_sid())
        for c in ("content", "body", "text"):
            if hasattr(row, c):
                setattr(row, c, content)
        ...
        db.session.rollback()
        try:
            current_app.logger.exception("group_post failed: %s", e)
        except Exception:
            pass
...
def group_add_link():
    ...
    try:
        ...
        db.session.rollback()
        try:
            current_app.logger.exception("group_add_link failed: %s", e)
        except Exception:
            pass
        ...
        rows = [
            {
                ...
                or (
                    "Broadcast"
                    if getattr(conv, "is_broadcast", False)
                    else (
                        "Group" if getattr(conv, "is_group", False) else "Conversation"
                    )
                ),
                "is_broadcast": bool(getattr(conv, "is_broadcast", False)),
                "is_group": bool(getattr(conv, "is_group", False)),
                "when": (
                    getattr(conv, "created_at", None).strftime("%b %d, %Y %I:%M %p")
                    if getattr(conv, "created_at", None)
                    else ""
                ),
            }
            for conv in q.all()
...
def public_profile(user_id):
    ...
    display_name = (
        user.display_name
        if viewer_is_owner or viewer_is_friend or user.is_discoverable_by_real_name()
        else None
    )
    alias_name = (
        user.alias_name if viewer_is_owner or user.is_discoverable_by_alias() else None
    )
    ...
    bundle = {
        ...
        "alias": alias_name,
        "afflictions": (
            [a.name for a in profile.afflictions]
            if profile.can_view_afflictions(viewer_is_owner, viewer_is_friend)
            else []
        ),
        "favorite_dispensary": (
            profile.favorite_dispensary.name
            if profile.show_favorite_dispensary(viewer_is_owner, viewer_is_friend)
            else None
        ),
        "qol_scores": (
            profile.qol_scores
            if profile.show_qol(viewer_is_owner, viewer_is_friend)
            else []
        ),
        "voting_history": (
            profile.voting_history
            if profile.show_voting(viewer_is_owner, viewer_is_friend)
            else []
        ),
        "favorites": (
            [f.name for f in profile.favorites]
            if profile.show_favorites(viewer_is_owner, viewer_is_friend)
            else []
        ),
        "recommendations": (
            profile.recommendations
            if profile.show_recommendations(viewer_is_owner, viewer_is_friend)
            else []
        ),
        "afflictions_over_time": (
            profile.afflictions_over_time
            if profile.show_afflictions_over_time(viewer_is_owner, viewer_is_friend)
            else []
        ),
